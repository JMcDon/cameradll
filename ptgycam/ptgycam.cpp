...
extern "C"
{
	...
	int _save_frame()
	{
		...
		fc2ConvertImageTo(FC2_PIXEL_FORMAT_RGBU, &g_cam->rawImage, &g_cam->saveImage);
		return 0;
	}


	__declspec(dllexport) int Create(){
		g_cam = (camera*)malloc(sizeof(camera)); //deallocated in CleanUp()
		int success = 0;
	...
	__declspec(dllexport) fc2Image GrabImage(){
